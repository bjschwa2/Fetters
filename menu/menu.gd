...
onready var tween = get_node("homeTween")
...
func reposition_screens():
	...
	if  scale > 1:
		get_node("backgroundLayer").set_scale(Vector2(scale,scale))
		
		if scale_vector.y / scale_vector.x < 1:
			get_node("backgroundLayer").set_offset(Vector2(0, (1 - scale_vector.x / scale_vector.y) * initial_size.y))
		else:
			get_node("backgroundLayer").set_offset(Vector2(0, 0))
	tween.remove_all()
